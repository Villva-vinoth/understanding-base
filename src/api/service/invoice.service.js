...
module.exports ={ 
    getAllInvoice : (data)=>{
        ...
        return transactions(async (t)=>{
            const invoices = await invoiceModel.findAndCountAll(pg,{transaction:t})
            return invoices;
        })
    },
    getInvoiceById : (data)=>{
        return transactions(async (t)=>{
            const invoice = await invoiceModel.findOne({where:{invoice_id:data.id},transaction:t})
            if(!invoice){
    ...
    getInvoiceByOrderId : (data)=>{
        return transactions(async (t)=>{
            const invoice = await invoiceModel.findOne({
                ...
                include:[{model:orderModel,as:'order_invoices'},{model:userModel,as:'user_invoices'}],
                transaction:t
            })
            if(!invoice){
                throw new NotFoundError("Invoice Not Found",404)
